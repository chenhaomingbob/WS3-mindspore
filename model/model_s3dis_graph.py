...
class RandLA_S3DIS_WithLoss(nn.Cell):
    ...
    def construct(self, feature, feature2, labels, input_inds, cloud_inds,
        ...
        ###
        # #
        # # SP_loss = self.get_sp_loss_by_mask(pred_embed,
        # #                                    logits,
        # #                                    one_hot_labels,
        # #                                    valid_mask,
        ...
        # topk = self.topk + num_invalid_points
        # # 点类别的数量
        # num_class = one_hot_labels.shape[1]  # scalar: 13
        #
        # # return 1
        # valid_one_hot_label = one_hot_labels * valid_mask.reshape(-1, 1)  # (B*N,13)
        # valid_embed = pred_embed * valid_mask.reshape(-1, 1)  # (B*N,32)
        # invalid_embed = pred_embed * invalid_mask.reshape(-1, 1)  # (B*N,32)
        ...
        # SP_loss = nn.SoftmaxCrossEntropyWithLogits()(logits, new_soft_label_hot)
        # SP_loss = SP_loss.sum() / num_new_valid_mask
        #
        # loss = CE_loss + SP_loss
        # loss = CE_loss + SP_loss
    ...
    # @staticmethod
    def get_sp_loss_by_mask(self, embed, logits, one_hot_labels, valid_mask, topk):
        """
