...
from data.S3DIS_dataset_test import dataloader, ms_map
from utils.tools import DataProcessing as DP
from utils.tools import ConfigS3DIS as cfg
from model.model_s3dis_remove_bias import RandLANet_S3DIS
from utils.logger import get_logger
from utils.helper_ply import write_ply

# from aaai_ws.S3DIS_dataset_test import dataloader, ms_map
# from aaai_ws.tools import ConfigS3DIS as cfg
# from aaai_ws.tools import DataProcessing as DP
# from aaai_ws.model_s3dis_v12 import RandLANet_S3DIS
# from aaai_ws.model_s3dis_v2 import RandLANet_S3DIS
...
def run_eval(args):
    ...
    for ckpt_i, ckpt in enumerate(ckpts):
        ...
        for step_i, data in enumerate(val_loader):
            ...
            prob_logits = ops.Softmax(-1)(logits)  # 0.0003s

            #####
            #  TODO 在Ascend环境上, prob_logits.asnumpy() 这个操作竟然要2.7s
            #####
            prob_logits = prob_logits.asnumpy()  # 要 2.72~2.8ls
            ...
            #       f"[train_modelarts_notebook_v2.py] post process 耗时: {post_process_time - network_time}s")
...
if __name__ == "__main__":
    ...
    dirs.add_argument('--dataset_dir', type=str, help='path of dataset', default='./datasets/S3DIS')

    dirs.add_argument('--outputs_dir', type=str, help='path of output', default='test_outputs')

    expr.add_argument('--float16', type=bool, default=False)
    ...
    # else:
    #     args.outputs_dir = os.path.join(f"test_{t}")
