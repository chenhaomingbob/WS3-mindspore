...
dataset_path = '/home/ubuntu/hdd1/mqh/dataset/s3dis/Stanford3dDataset_v1.2_Aligned_Version'
...
sub_grid_size = 0.04
original_pc_folder = join(dirname(dataset_path), 'original_npy')
sub_pc_folder = join(dirname(dataset_path), 'train_{:.3f}'.format(sub_grid_size))
os.mkdir(original_pc_folder) if not exists(original_pc_folder) else None
os.mkdir(sub_pc_folder) if not exists(sub_pc_folder) else None
out_format = '.npy'


def convert_pc2npy(anno_path, save_path):
    """
    ...
    #print('xyz.shape:',xyz.shape,' colors.shape:',colors.shape,' labels.shape:',labels.shape)
    np.save(save_path, np.concatenate((xyz, colors, labels), axis=1).T)
...
if __name__ == '__main__':
    ...
    for annotation_path in anno_paths:
        ...
        convert_pc2npy(annotation_path, join(original_pc_folder, out_file_name))
