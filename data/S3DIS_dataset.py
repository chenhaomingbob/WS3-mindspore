...
class S3DISDatasetGenerator:
    def __init__(self, dataset_dir, labeled_point_percent=1):
    ...
    def spatially_regular_gen(self):
        ...
        for i in range(self.n_samples * self.batch_size):  # num_per_epoch
            ...
            if len(points) < self.cfg.num_points:
                ...
                queried_idx = \
                    self.dataset.input_trees[self.split][cloud_idx].query(pick_point, k=self.cfg.num_points)[1][0]
            ...
            #       f"[S3DIS_dataset.py] sample{i} 耗时: {time.time() - begin_time}s")
...
def ms_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx, batchInfo):
    ...
    # print(strftime("%Y-%m-%d %H:%M:%S", localtime()) +
    #       f"[S3DIS_dataset.py] ms_map 耗时: {time.time() - begin_time}s")

    return batch_features, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx, \
...
def dataloader(cfg, **kwargs):
    dataset = S3DISDatasetGenerator(cfg.dataset_dir, cfg.labeled_percent)
    ...
    return ds.GeneratorDataset(train_sampler, input_columns, **kwargs), \
           ds.GeneratorDataset(val_sampler, input_columns, **kwargs), \
           dataset
