...
class S3DISDatasetGenerator:
    def __init__(self, dataset_dir):
        ...
        self.val_area = 'Area_5'
        self.original_ply_path = self.dataset_path / 'original_ply'
        self.paths = list(self.original_ply_path.glob('*.ply'))
    ...
    def load_data(self):
        ...
        for i, file_path in enumerate(self.paths):
            ...
            kd_tree_file = tree_path / '{:s}_KDTree.pkl'.format(cloud_name)
            sub_ply_file = tree_path / '{:s}.ply'.format(cloud_name)
        ...
        for i, file_path in enumerate(self.paths):
            ...
            if self.val_area in cloud_name:
                proj_file = tree_path / '{:s}_proj.pkl'.format(cloud_name)
...
def ms_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx, batchInfo):
    ...
    return batch_features, batch_labels, batch_pc_idx, batch_cloud_idx, input_points[0], input_points[1], input_points[
        2], input_points[3], input_points[4], \
           input_neighbors[0], input_neighbors[1], input_neighbors[2], input_neighbors[3], input_neighbors[4], \
           input_pools[0], input_pools[1], input_pools[2], \
           input_pools[3], input_pools[4], input_up_samples[0], input_up_samples[1], input_up_samples[2], \
           input_up_samples[3], input_up_samples[4]


def dataloader(dataset_dir, **kwargs):
    dataset = S3DISDatasetGenerator(dataset_dir)
    ...
    return ds.GeneratorDataset(train_sampler, ["xyz", "colors", "labels", "q_idx", "c_idx"],
                               **kwargs), \
           ds.GeneratorDataset(val_sampler,
                               ["xyz", "colors", "labels", "q_idx",
                                "c_idx"], **kwargs), dataset
...
if __name__ == '__main__':
    ...
    val_loader = val_ds.batch(batch_size=cfg.val_batch_size,
                              per_batch_map=ms_map,
                              input_columns=["xyz", "colors", "labels", "q_idx", "c_idx"],
                              output_columns=["features", "labels", "input_inds", "cloud_inds",
                                              "p0", "p1", "p2", "p3", "p4",
